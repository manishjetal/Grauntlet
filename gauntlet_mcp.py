...
def refresh_fyers_token() -> str:
    ...
    try:
        ...
        elif "data" in r4_data and "auth" in r4_data["data"]:
            auth_code = r4_data["data"]["auth"]
        else:
            return json.dumps({"ok": False, "error": f"fyers /token step: unrecognized response shape: {r4_data}"})
